...
result <- db$query_db("SELECT * FROM rankings")
...
```{r echo=FALSE}
latest_year <- max(result_tibble$year)
top_countries <-
  result_tibble %>%
...
p_brics <- 
  ggplot(filtered_data %>% 
         ...
         aes(x = year, y = hs_eci_rank, group = code, color = code)) +
  geom_line() +
...
animate(p_brics, nframes = 25,end_pause = 3,duration=10, renderer = gifski_renderer(paste0('output/eci_ranking-1995-2020-change.gif')))
